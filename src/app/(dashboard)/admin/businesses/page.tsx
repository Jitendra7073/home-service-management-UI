...
export default function AdminBusinessesPage() {
  ...
  const { data: businesses = [], isLoading } = useBusinesses(params);

  // Mutations
  const approveMutation = useApproveBusiness();
  const rejectMutation = useRejectBusiness();
  const blockBusinessMutation = useBlockBusiness();
  const unblockBusinessMutation = useUnblockBusiness();
  ...
  const handleBlockBusiness = async (reason: string) => {
    ...
    try {
      await blockBusinessMutation.mutateAsync({
  ...
  const handleUnblockBusiness = async (businessId: string) => {
    try {
      await unblockBusinessMutation.mutateAsync(businessId);
  ...
  const categories = Array.from(
    new Set(businesses.map((b: any) => b.category.name))
  );

  const isMutationPending =
    approveMutation.isPending ||
    rejectMutation.isPending ||
    blockBusinessMutation.isPending ||
    unblockBusinessMutation.isPending;
  ...
  return (
    <div className="space-y-6">
            ...
            <BusinessCard
              ...
              onUnblock={
                business.isRestricted
                  ? () => handleUnblockBusiness(business._id)
                  : undefined
              }
      ...
      <BlockDialog
        ...
        entityName={selectedBusiness?.name || ""}
        description={`You are about to block ${selectedBusiness?.name || ""}. This will restrict access to this business and all its services.`}
      />
    </div>
  );
}
